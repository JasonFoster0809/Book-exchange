...
/**
 * ImageGallery: Thư viện ảnh với hiệu ứng Zoom
 */
const ImageGallery = ({ images, status }: { images: string[], status: ProductStatus }) => {
  ...
  return (
    <div className="flex flex-col gap-4 animate-slide-up" style={{ animationDelay: '0ms' }}>
      {/* Main Stage */}
      <div 
        className="aspect-square relative rounded-[2rem] border border-gray-100 bg-white overflow-hidden group shadow-sm cursor-zoom-in"
        onMouseEnter={() => setIsHovering(true)}
        onMouseLeave={() => setIsHovering(false)}
      >
        <img 
          src={mainImage} 
          className={`w-full h-full object-contain mix-blend-multiply transition-transform duration-700 ease-out ${isHovering ? 'scale-110' : 'scale-100'}`} 
          alt="Product Detail" 
        />
        
        {/* Status Badge */}
        {status === 'sold' && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-10 animate-fade-in">
        ...
        <div className="absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-xs font-bold text-gray-500 shadow-sm opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
          Phóng to
        </div>
      </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 hide-scrollbar">
          {images.map((img, idx) => (
            <button 
              ...
              className={`w-20 h-20 rounded-2xl border-2 flex-shrink-0 overflow-hidden transition-all duration-300 ${mainImage === img ? 'border-[#00418E] ring-2 ring-blue-100 scale-105' : 'border-transparent opacity-60 hover:opacity-100 hover:scale-105'}`}
            >
...
/**
 * SellerInfoCard: Thông tin người bán
 */
const SellerInfoCard = ({ seller, isUntrusted }: { seller: User, isUntrusted: boolean }) => (
  <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
    <div className="relative shrink-0">
      <img 
        src={seller.avatar || 'https://via.placeholder.com/100'} 
        className={`w-16 h-16 rounded-full border-2 p-0.5 object-cover transition-colors duration-300 ${isUntrusted ? 'border-red-500' : 'border-[#00418E]'}`} 
        alt="avatar" 
      />
      {isUntrusted ? (
        <div className="absolute -bottom-1 -right-1 bg-red-600 text-white p-1 rounded-full animate-pulse shadow-sm border-2 border-white">
          <ShieldAlert size={12}/>
        </div>
      ) : seller.isVerified && (
        <div className="absolute -bottom-1 -right-1 bg-blue-500 text-white p-1 rounded-full shadow-sm border-2 border-white">
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2">
        <h4 className="font-bold text-gray-900 truncate text-lg">{seller.name}</h4>
      ...
      <div className="flex gap-4 mt-3">
        <Link to={`/profile/${seller.id}`} className="text-xs font-bold text-[#00418E] hover:underline flex items-center gap-1 group">
          <Store size={14} className="group-hover:scale-110 transition-transform"/> Xem Shop
        </Link>
        {!isUntrusted && (
          <span className="text-xs text-gray-400 flex items-center gap-1"><Star size={12} className="text-yellow-400 fill-yellow-400"/> 4.9/5.0</span>
        )}
      </div>
    </div>
  </div>
...
const DetailSkeleton = () => (
  <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
    <div className="lg:col-span-7 space-y-4">
      <div className="aspect-square rounded-[2rem] animate-shimmer w-full"></div>
      <div className="flex gap-3">
        {[1,2,3,4].map(i => <div key={i} className="w-20 h-20 rounded-2xl animate-shimmer"></div>)}
      </div>
    </div>
    <div className="lg:col-span-5 space-y-6">
      <div className="h-8 w-3/4 rounded-lg animate-shimmer"></div>
      <div className="h-6 w-1/2 rounded-lg animate-shimmer"></div>
      <div className="h-40 w-full rounded-[2rem] animate-shimmer"></div>
      <div className="h-24 w-full rounded-2xl animate-shimmer"></div>
    </div>
  </div>
);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const mappedProduct: Product = { 
        ...
        images: pData.images || [], 
        status: pData.status as ProductStatus, // Fix Error Type
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-24 font-sans text-[#1E293B]">
      ...
      <div className="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-40 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-gray-500 overflow-hidden animate-slide-up">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            {/* Desktop Description */}
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 hidden lg:block animate-slide-up" style={{animationDelay: '200ms'}}>
              <h3 className="text-lg font-black text-[#00418E] uppercase tracking-widest mb-6 flex items-center gap-2">
                <Box size={20}/> Mô tả chi tiết
              </h3>
              <div className="prose prose-blue max-w-none text-gray-600 whitespace-pre-wrap leading-relaxed text-base">
                {product.description}
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative">
            <div className="sticky top-24 space-y-6 animate-slide-up" style={{animationDelay: '300ms'}}>
              ...
              <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-lg shadow-blue-900/5 border border-gray-100 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <span className="bg-blue-50 text-[#00418E] text-xs font-bold px-3 py-1.5 rounded-lg uppercase tracking-wider border border-blue-100">
                      {product.category}
                    </span>
                    <button onClick={handleToggleLike} className="group transition-transform active:scale-95">
                      <Heart size={26} className={`transition-all duration-300 ${isLiked ? 'fill-red-500 text-red-500 scale-110 drop-shadow-md' : 'text-gray-300 group-hover:text-red-400'}`} />
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-[#F8FAFC] p-4 rounded-2xl border border-gray-100">
                      <p className="text-[10px] text-gray-400 uppercase font-black mb-1 tracking-wider">Tình trạng</p>
                      <p className="font-bold text-gray-900">{product.condition}</p>
                    </div>
                    <div className="bg-[#F8FAFC] p-4 rounded-2xl border border-gray-100">
                      <p className="text-[10px] text-gray-400 uppercase font-black mb-1 tracking-wider">Giao dịch</p>
                      <p className="font-bold text-gray-900">{product.tradeMethod === 'direct' ? 'Trực tiếp' : 'Ship COD'}</p>
                    </div>
                  </div>
                  ...
                  <div className="hidden lg:flex flex-col gap-3">
                    <button 
                      ...
                      className={`w-full py-4 rounded-xl font-black text-lg shadow-lg flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-95 ${
                    ...
                    <button 
                      ...
                      disabled={isViewerUntrusted || isSellerUntrusted}
                      className="w-full py-4 bg-white border-2 border-[#00418E] text-[#00418E] rounded-xl font-bold hover:bg-blue-50 transition-all flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95"
                    >
              ...
              <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100">
                <h3 className="font-bold text-[#00418E] flex items-center gap-2 text-sm uppercase mb-4 tracking-wider">
                  <ShieldCheck size={18}/> Mẹo an toàn
                </h3>
                <ul className="space-y-3 text-sm text-gray-600 font-medium">
                  <li className="flex items-start gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-blue-400 mt-1.5 shrink-0"></div>
                    <span>KHÔNG chuyển khoản trước (cọc) khi chưa kiểm tra hàng.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-blue-400 mt-1.5 shrink-0"></div>
                    <span>Nên giao dịch trực tiếp tại các khu vực đông người trong trường (Sảnh H6, Thư viện).</span>
                  </li>
                </ul>
              </div>
            </div>
        ...
        {/* 4. COMMENTS SECTION */}
        <div className="mt-16 max-w-4xl animate-slide-up" style={{animationDelay: '400ms'}}>
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-2xl font-black text-gray-900 flex items-center gap-3">
              Hỏi đáp <span className="bg-gray-100 text-gray-500 text-sm font-bold px-3 py-1 rounded-full">{comments.length}</span>
            </h3>
          </div>
          ...
          {isViewerUntrusted ? (
            ...
            <form onSubmit={handleSendComment} className="flex gap-4 mb-10 group">
              <img src={currentUser.avatar} className="w-12 h-12 rounded-full border-2 border-white shadow-md object-cover" alt="me"/>
              <div className="flex-1 relative">
                <textarea 
                  value={newComment}
                  onChange={e => setNewComment(e.target.value)}
                  placeholder="Đặt câu hỏi cho người bán..."
                  className="w-full bg-white border border-gray-200 rounded-2xl p-4 pr-14 focus:outline-none focus:border-[#00418E] focus:ring-4 focus:ring-blue-50 transition-all min-h-[60px] resize-y shadow-sm"
                />
                <button 
                  disabled={!newComment.trim() || submitting}
                  className="absolute right-3 bottom-3 p-2 bg-[#00418E] text-white rounded-xl hover:bg-[#003370] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md active:scale-95"
                >
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-8 text-center mb-10">
              <p className="text-blue-800 font-bold mb-2">Bạn cần đăng nhập để tham gia thảo luận</p>
              <Link to="/auth" className="inline-block mt-2 text-sm font-black text-white bg-[#00418E] px-6 py-2 rounded-xl hover:bg-[#003370] transition-all shadow-lg hover:shadow-xl">Đăng nhập ngay</Link>
            </div>
          )}

          <div className="space-y-6">
            {comments.map((c, idx) => (
              <div key={c.id} className="flex gap-4 animate-slide-up" style={{animationDelay: `${idx * 50}ms`}}>
                <img src={c.userAvatar} className="w-10 h-10 rounded-full border border-gray-100 object-cover" alt="user"/>
                <div className="flex-1 bg-white p-5 rounded-2xl rounded-tl-none border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {c.userId === seller?.id && <span className="bg-[#00418E] text-white text-[10px] px-2 py-0.5 rounded font-black uppercase tracking-wider">Người bán</span>}
                    </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-24 border-t border-gray-200 pt-16 animate-slide-up" style={{animationDelay: '500ms'}}>
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-black text-gray-900">Có thể bạn cũng thích</h3>
              <Link to="/market" className="text-[#00418E] font-bold text-sm hover:underline">Xem thêm</Link>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 lg:hidden z-50 safe-area-pb shadow-[0_-10px_30px_rgba(0,0,0,0.1)] animate-slide-up">
        <div className="flex gap-3">
          <button 
            onClick={() => !(isViewerUntrusted || isSellerUntrusted) && navigate(`/chat?partnerId=${seller?.id}`)}
            disabled={isViewerUntrusted || isSellerUntrusted}
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-[2rem] w-full max-w-sm overflow-hidden shadow-2xl animate-slide-up">
            <div className="bg-red-50 p-6 border-b border-red-100 flex justify-between items-center">
              ...
              <button onClick={() => setShowReportModal(false)} className="text-red-400 hover:text-red-600 p-1 hover:bg-red-100 rounded-full transition-colors"><X size={20}/></button>
            </div>
            <form onSubmit={handleReport} className="p-6 space-y-5">
              ...
              <button className="w-full bg-red-600 text-white py-4 rounded-xl font-black hover:bg-red-700 transition-all shadow-lg shadow-red-200 active:scale-95 uppercase tracking-widest text-sm">
                GỬI BÁO CÁO
              </button>
            </form>
          </div>
