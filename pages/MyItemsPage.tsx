...
import ProductSkeleton from '../components/ProductSkeleton'; // Đã dùng chung
...
const VisualEngine = () => (
  <style>{`
    ...
    .text-gradient { background: linear-gradient(135deg, #00418E 0%, #3B82F6 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
  `}</style>
);
...
const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message, isDanger }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-sm transition-all animate-in fade-in zoom-in-95 duration-200">
      <div className="relative bg-white rounded-[2rem] w-full max-w-sm p-6 shadow-2xl border border-white/50">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button onClick={() => { onConfirm(); onClose(); }} className={`py-3.5 rounded-xl font-bold text-white shadow-lg hover:shadow-xl hover:translate-y-[-2px] transition-all ${isDanger ? 'bg-red-500 shadow-red-200' : 'bg-[#00418E] shadow-blue-200'}`}>Xác nhận</button>
        </div>
      </div>
    </div>
  );
...
const MyItemsPage: React.FC = () => {
  ...
  useEffect(() => { if (!user) navigate('/auth'); else fetchData(); }, [user]);

  const fetchData = async () => {
    setLoading(true);
    try {
      const { data: myData } = await supabase.from('products').select('*, seller:profiles!seller_id(name, avatar_url, verified_status)').eq('seller_id', user!.id).order('created_at', { ascending: false });
      const { data: savedData } = await supabase.from('saved_products').select(`product:products (*, seller:profiles!seller_id(name, avatar_url, verified_status))`).eq('user_id', user!.id);
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="mb-10 animate-enter">
          <h1 className="text-3xl font-black text-slate-800 mb-6 flex items-center gap-3">
            <LayoutDashboard className="text-[#00418E]" size={32}/> <span className="text-gradient">Góc Của Tôi</span>
          </h1>
          {loading ? (
             // SKELETON CHO HEADER
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               {[1,2,3].map(i => <div key={i} className="bg-white h-24 rounded-[20px] animate-pulse"/>)}
             </div>
          ) : (
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden animate-enter" style={{animationDelay: '0.1s'}}>
          <div className="flex border-b border-slate-100 bg-slate-50/50">
            {[
              ...
              <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex-1 py-5 flex items-center justify-center gap-2 text-sm font-bold transition-all relative ${activeTab === tab.id ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                <tab.icon size={18} className={activeTab === tab.id ? tab.color : ''} /> {tab.label} <span className="bg-slate-100 px-2 py-0.5 rounded-full text-xs">{tab.count}</span>
          ...
          <div className="p-6 md:p-8 min-h-[400px]">
            {loading ? (
              // SKELETON LOADING (DÙNG CHUNG)
              <div className="space-y-5">
                {[1, 2, 3].map((i) => <ProductSkeleton key={i} viewMode="list" />)}
              </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center py-20">
                ...
                {activeTab === 'selling' && (
                  <Link to="/post" className="bg-[#00418E] text-white px-8 py-3 rounded-2xl font-bold shadow-lg shadow-blue-200 hover:translate-y-[-2px] transition-all flex items-center gap-2">
                ...
                {currentItems.map((item) => (
                  <div key={item.id} className="group bg-white border border-slate-100 rounded-3xl p-4 hover:border-blue-200 hover:shadow-lg transition-all flex flex-col sm:flex-row gap-5">
                    <div className="w-full sm:w-36 h-36 flex-shrink-0 bg-slate-100 rounded-2xl overflow-hidden relative">
                      ...
                      {item.status === ProductStatus.SOLD && <div className="absolute inset-0 bg-black/50 flex items-center justify-center backdrop-blur-sm"><span className="text-white text-xs font-black border border-white px-3 py-1 rounded-full uppercase tracking-widest">Đã Bán</span></div>}
                    </div>
                    <div className="flex-1 flex flex-col justify-between py-1">
                      ...
                      <div className="flex items-center justify-end gap-2 border-t border-slate-50 pt-4 mt-2">
                        {activeTab === 'selling' ? (
                          <>
                            <button onClick={() => navigate(`/edit-item/${item.id}`)} className="p-2.5 rounded-xl bg-slate-50 text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition-colors"><Edit3 size={18}/></button>
                            <button onClick={() => toggleStatus(item)} className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-colors ${item.status === ProductStatus.SOLD ? 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>
                              {item.status === ProductStatus.SOLD ? <RotateCcw size={16}/> : <CheckCircle2 size={16}/>} {item.status === ProductStatus.SOLD ? 'Đăng lại' : 'Đã bán'}
                            </button>
                            <button onClick={() => openDeleteModal(item.id, false)} className="p-2.5 rounded-xl bg-red-50 text-red-500 hover:bg-red-100 transition-colors"><Trash2 size={18}/></button>
                          </>
                        ) : (
                          <button onClick={() => openDeleteModal(item.id, true)} className="px-4 py-2.5 rounded-xl bg-red-50 text-red-500 hover:bg-red-100 text-xs font-bold flex items-center gap-2 transition-colors"><Heart size={16} className="fill-current"/> Bỏ lưu</button>
                        )}
                        <div className="w-[1px] h-6 bg-slate-200 mx-2"></div>
                        <Link to={`/product/${item.id}`} className="px-5 py-2.5 rounded-xl bg-[#00418E] text-white text-xs font-bold hover:shadow-lg hover:shadow-blue-200 transition-all flex items-center gap-1">Xem <ChevronRight size={14}/></Link>
                      </div>
                    </div>
                  </div>
                ))}
