...
type FormState = {
  ...
  category: ProductCategory; condition: ProductCondition; tradeMethod: TradeMethod;
...
const PostItemPage: React.FC = () => {
  ...
  const clearDraft = () => { localStorage.removeItem('post_draft'); setShowSaveDraft(false); };
  ...
  return (
    <div className="min-h-screen pb-32 font-sans text-slate-800">
      <VisualEngine />
      <div className="fixed inset-0 z-[-1] overflow-hidden pointer-events-none"><div className="absolute top-0 left-0 w-full h-[600px] bg-gradient-to-b from-blue-50/50 to-transparent"></div></div>
      ...
      {showSaveDraft && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 animate-enter border border-slate-700 w-[90%] max-w-lg">
          <div className="flex-1"><p className="font-bold text-sm">Phát hiện bản nháp chưa lưu</p><p className="text-xs text-slate-400">Bạn có muốn khôi phục?</p></div>
          <div className="flex gap-3"><button onClick={clearDraft} className="px-4 py-2 hover:bg-white/10 rounded-lg text-xs font-bold transition">Hủy bỏ</button><button onClick={restoreDraft} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-xs font-bold transition shadow-lg">Khôi phục</button></div>
        </div>
      )}
      ...
      <div className="max-w-5xl mx-auto px-4 pt-12">
        <div className="flex justify-between items-end mb-12 animate-enter">
          <div><h1 className="text-4xl md:text-5xl font-black text-[#00418E] mb-3 tracking-tight">{editId ? 'Hiệu Chỉnh' : 'Đăng Tin Mới'}</h1><p className="text-slate-500 font-medium flex items-center gap-2"><Zap size={16} className="text-yellow-500"/> AI hỗ trợ điền thông tin tự động</p></div>
        ...
        {step === 1 && (
          <div className="glass-panel p-8 rounded-[2.5rem] animate-enter">
            <div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-bold flex items-center gap-3"><Camera className="text-blue-600"/> Thư viện ảnh</h3><span className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-bold">{previewUrls.length}/8 ảnh</span></div>
                ...
                <div className="w-full aspect-[4/3] bg-white rounded-2xl shadow-sm border border-slate-200 mb-4 overflow-hidden relative group">
                  {previewUrls.length > 0 ? (
                    <>
                      ...
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 opacity-0 group-hover:opacity-100 transition-all bg-white/90 backdrop-blur p-2 rounded-xl shadow-lg border border-slate-100"><button className="p-2 hover:bg-slate-100 rounded-lg"><RotateCcw size={18}/></button><button className="p-2 hover:bg-slate-100 rounded-lg"><Crop size={18}/></button><button className="p-2 hover:bg-slate-100 rounded-lg"><Layers size={18}/></button></div>
                    </>
                  ) : (
                    <div className="flex flex-col items-center justify-center h-full text-slate-400"><ImageIcon size={48} className="mb-2 opacity-20"/><p className="text-sm">Chưa có ảnh nào được chọn</p></div>
                  )}
                </div>
              </div>
            </div>
            <div className="flex justify-end mt-8"><button onClick={() => { if(previewUrls.length===0) return addToast("Cần ít nhất 1 ảnh","warning"); setStep(2); }} className="px-8 py-3.5 bg-[#00418E] text-white rounded-xl font-bold flex items-center gap-2 hover:bg-[#003370] transition shadow-xl shadow-blue-900/20 active:scale-95">Tiếp tục <ArrowRight size={18}/></button></div>
          </div>
        )}

        {step === 2 && (
          <div className="glass-panel p-8 rounded-[2.5rem] animate-enter">
            <div className="flex justify-between items-center mb-8"><h3 className="text-2xl font-bold flex items-center gap-3"><FileText className="text-blue-600"/> Chi tiết sản phẩm</h3><button onClick={analyzeImageAI} className="px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg hover:scale-105 transition-transform"><Wand2 size={16}/> AI Auto-Fill</button></div>
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
              <div className="md:col-span-8 space-y-6">
                <div><label className="block text-xs font-extrabold text-slate-500 uppercase mb-2 ml-1">Tiêu đề tin đăng</label><input value={state.title} onChange={e => dispatch({type:'SET_FIELD', field:'title', value:e.target.value})} className="input-modern text-lg" placeholder="VD: Giáo trình Giải tích 1 - NXB ĐHQG"/></div>
                <div className="grid grid-cols-2 gap-6">
                  <div><label className="block text-xs font-extrabold text-slate-500 uppercase mb-2 ml-1">Danh mục</label><select value={state.category} onChange={e => dispatch({type:'SET_FIELD', field:'category', value:e.target.value})} className="input-modern appearance-none cursor-pointer">{CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.icon} {c.label}</option>)}</select></div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      {priceAnalysis && <div className={`absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold px-2 py-0.5 rounded ${priceAnalysis === 'low' ? 'bg-green-100 text-green-700' : priceAnalysis === 'high' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>{priceAnalysis === 'low' ? 'Giá tốt' : priceAnalysis === 'high' ? 'Giá cao' : 'Hợp lý'}</div>}
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="bg-[#F8FAFC] border-2 border-slate-200 rounded-2xl overflow-hidden transition-colors focus-within:border-[#00418E] focus-within:bg-white">
                    <div className="flex gap-1 p-2 border-b border-slate-200 bg-slate-50">{['B', 'I', 'U', 'List', 'Link'].map(t => <button key={t} className="rich-toolbar-btn hover:bg-white text-xs font-bold px-3 py-1.5">{t}</button>)}</div>
              ...
              <div className="md:col-span-4 space-y-6">
                <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm"><label className="block text-xs font-extrabold text-slate-500 uppercase mb-3">Tình trạng</label><div className="space-y-2">{CONDITIONS.map(c => <button key={c.value} onClick={() => dispatch({type:'SET_FIELD', field:'condition', value:c.value})} className={`w-full p-3 rounded-xl text-left text-sm font-bold border-2 transition-all flex items-center justify-between ${state.condition === c.value ? c.color : 'border-transparent bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>{c.label} {state.condition === c.value && <CheckCircle2 size={16}/>}</button>)}</div></div>
                <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm"><label className="block text-xs font-extrabold text-slate-500 uppercase mb-3">Tags</label><div className="flex flex-wrap gap-2 mb-3">{state.tags.map(t => <span key={t} className="px-2 py-1 bg-blue-50 text-blue-700 text-xs font-bold rounded-lg flex items-center gap-1">#{t} <button onClick={() => dispatch({type:'REMOVE_TAG', tag:t})}><X size={12}/></button></span>)}</div><input onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); const val = e.currentTarget.value.trim(); if(val) { dispatch({type:'ADD_TAG', tag:val}); e.currentTarget.value = ''; }}}} placeholder="+ Thêm tag..." className="w-full bg-slate-50 border-none rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-100"/></div>
              </div>
            </div>
            <div className="flex justify-between mt-8"><button onClick={() => setStep(1)} className="px-6 py-3 text-slate-500 font-bold hover:bg-slate-100 rounded-xl transition flex items-center gap-2"><ArrowLeft size={18}/> Quay lại</button><button onClick={() => { if(!state.title || !state.price) return addToast("Điền đủ thông tin!","warning"); setStep(3); }} className="px-8 py-3.5 bg-[#00418E] text-white rounded-xl font-bold flex items-center gap-2 hover:bg-[#003370] transition shadow-xl shadow-blue-900/20 active:scale-95">Tiếp tục <ArrowRight size={18}/></button></div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="glass-panel p-8 rounded-[2.5rem] animate-enter">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              <div className="space-y-4">
                ...
                <div className="flex gap-4">
                  {[ {val: TradeMethod.DIRECT, icon: <MapPin/>, label: 'Gặp trực tiếp', desc: 'An toàn, không phí'}, {val: TradeMethod.SHIPPING, icon: <Box/>, label: 'Giao hàng', desc: 'Tiện lợi, có phí ship'} ].map((m: any) => (
            ...
            <div className="flex justify-between mt-8"><button onClick={() => setStep(2)} className="px-6 py-3 text-slate-500 font-bold hover:bg-slate-100 rounded-xl transition flex items-center gap-2"><ArrowLeft size={18}/> Quay lại</button><button onClick={() => setStep(4)} className="px-8 py-3.5 bg-[#00418E] text-white rounded-xl font-bold flex items-center gap-2 hover:bg-[#003370] transition shadow-xl shadow-blue-900/20 active:scale-95">Tiếp tục <ArrowRight size={18}/></button></div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-enter">
            ...
            <div className="glass-panel p-8 rounded-[2.5rem] flex flex-col">
              ...
              <div className="space-y-4 mb-8">
                <div className="flex items-center gap-4 p-4 bg-blue-50 rounded-2xl border border-blue-100"><div className="p-2 bg-white rounded-xl text-blue-600 shadow-sm"><Search size={20}/></div><div><p className="font-bold text-sm text-blue-900">SEO Score: 95/100</p><p className="text-xs text-blue-700">Tin đăng của bạn rất chi tiết, dễ dàng tìm thấy.</p></div></div>
                <div className="flex items-center gap-4 p-4 bg-green-50 rounded-2xl border border-green-100"><div className="p-2 bg-white rounded-xl text-green-600 shadow-sm"><ShieldCheck size={20}/></div><div><p className="font-bold text-sm text-green-900">An toàn</p><p className="text-xs text-green-700">Nội dung hợp lệ, không vi phạm chính sách.</p></div></div>
              </div>
