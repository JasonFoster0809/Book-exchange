...
const ProfilePage: React.FC = () => {
  ...
  // --- HÀM LẤY REVIEW (ĐÃ SỬA: ĐƯỢC ĐỊNH NGHĨA TRƯỚC KHI DÙNG) ---
  const fetchReviews = async () => {
    if (!targetUserId) return;
    try {
      // Giả sử bảng reviews có quan hệ với profiles qua reviewer_id
      ...
      if (data) {
        // Map dữ liệu về đúng format Review
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Gọi hàm fetchReviews đã định nghĩa ở trên
        await fetchReviews();
